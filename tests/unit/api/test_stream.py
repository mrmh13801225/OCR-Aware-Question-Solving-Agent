...
def _event(state: RunState, index: int = 0) -> RunEvent:
...
def test_stream_from_after_skips_replayed_prefix() -> None:
    ...
    for state in ("SOLVE", "VERIFY", "DONE"):
        log.on_event(_event(state), run_id="r1")  # type: ignore[arg-type]
